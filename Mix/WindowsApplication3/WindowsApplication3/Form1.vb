...
Public Class Form1
    ...
    Sub tampilalamat()
        Try
            Call connectt()
            da = New MySqlDataAdapter("select * from gaji order by nip asc", conn)
            ds = New DataSet
            da.Fill(ds, "gaji")
    ...
    Sub simpan()
        Try
            Call connectt()
            Dim str As String
            str = "insert into gaji(nip,no_slip,tanggal,periode,gaji) values('" & txtpos.Text & "','" & txtkel.Text & "','" & txtkec.Text & "','" & txtkab.Text & "','" & txtpro.Text & "')"
    ...
    Sub ubah()
        Try
            Call connectt()
            Dim str As String
            str = "update gaji set nip='" & txtpos.Text & "',no_slip='" & txtkel.Text & "',tanggal='" & txtkec.Text & "',periode='" & txtkab.Text & "',gaji='" & txtpro.Text & "' where nip='" & txtpos.Text & "'"
            cmd = New MySqlCommand(str, conn)
            cmd.ExecuteNonQuery()
            MessageBox.Show("Berhasil", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Catch ex As Exception
    ...
    Sub hapus()
        Try
            Call connectt()
            Dim str As String
            str = "delete from gaji where nip='" & txtpos.Text & "'"
            cmd = New MySqlCommand(str, conn)
            cmd.ExecuteNonQuery()
            MessageBox.Show("Berhasil", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Catch ex As Exception
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.GajiTableAdapter.Fill(Me.PegawaiDataSet.gaji)
